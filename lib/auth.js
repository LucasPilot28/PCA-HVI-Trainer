...
// Comparación en tiempo constante para no filtrar por timing si el hash
// calculado y el guardado difieren.
...
export async function verifyPassword(password, storedHashHex, storedSaltHex, iterations) {
  ...
  return timingSafeEqualHex(bufToHex(bits), storedHashHex);
}
...
export async function getSession(db, request) {
  ...
  const row = await db
    .prepare(
      `SELECT sessions.user_id AS user_id, sessions.expires_at AS expires_at, users.username AS username
